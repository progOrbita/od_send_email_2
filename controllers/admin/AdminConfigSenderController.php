...
class AdminConfigSenderController extends ModuleAdminController
{
    ...
    public function mailSender(int $lang, string $name, string $mail, int $id, bool $is_customer): string
    {
        ...
        if (!SendEmail::sendUserMail($lang, $name, $mail)) {
            return $this->module->displayError($this->module->l('Error al realizar el envio trans'));
        }

        if (!ControlMails::insert($id, $is_customer)) {
            return $this->module->displayError($this->module->l('Error al almacenar datos en la db'));
        }
        ...
        return $this->module->displayConfirmation($this->module->l('Correo enviado'));
    }
    ...
    public function checkDate()
    {
        ...
        if ($date[0] != $last_date[0]) {
            return $this->updateCNT(0, $this->module->l('dia'));
        }

        $hour = explode(':', $date[1]);
        $last_hour = explode(':', $last_date[1]);
        if ($hour[0] != $last_hour[0]) {
            return $this->updateCNT(0, $this->module->l('hora'));
        }
        ...
        $this->displayErrors[] = $this->module->l('Error límite de correos alcanzado');
        return false;
    }
    ...
    public function updateCNT(int $value, string $time = '')
    {
        if (!Configuration::updateValue('_OD_SEND_EMAIL_CNT_', $value)) {
            $this->displayErrors[] = $this->module->l('Error al actualizar el contador cuando ' . $time . ' iguales.');
            return false;
        }
        return true;
    }
